...
if submitted:
    try:
        validar_parametros_entrada(
            ca=ca, mg=mg, k=k, al=al, p_mehlich=p,
            produtividade=produtividade, prnt=prnt_percent / 100
        )
    ...
    calagem = calcular_necessidade_calagem(
        dados_cultura["v_percent_desejado"], v_atual, ctc, prnt_percent / 100
    )
